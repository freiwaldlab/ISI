function confPstate_SphericalBar
    ...
    Pstate.param{sh(Pstate.param)+1} = {'BarThickness', 'int', 10, 0, ' deg'};
    ...
    Pstate.param{sh(Pstate.param)+1} = {'CheckSize', 'float', 5, 0, ' deg'};
    Pstate.param{sh(Pstate.param)+1} = {'FlickerRate', 'float', 5, 0, ' Hz'};
    ...
    Pstate.param{sh(Pstate.param)+1} = {'ScreenScaleFactor', 'int', 1, 0, ' x'};
