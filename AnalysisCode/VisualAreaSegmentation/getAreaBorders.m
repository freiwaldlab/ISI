...
% this script also generates overlays of azi/alt & blood vessels as well as resp mag maps
generatekret(anim,azi_expt,alt_expt,LP)

dimLP = size(LP);

for iLP = 1:dimLP(2)
    
    kmapfilename = strcat(SaveDir, 'Kmaps\', anim, '_LP', num2str(LP(iLP)), ...
        '_Thresh_0.05_kret.mat')
    load(kmapfilename);
    ...
    %Should be vert-hor, but the gradient in Matlab for y is opposite.
    ...
    %threshold visual field sign map at +/-1.5sig
    ...
    %Get point in visual space at the center of V1
    AreaInfo.Vcent(1) = kmap_hor(round(CoMxy(V1id,2)), round(CoMxy(V1id,1)));
    ...
    %Eccentricity
    AreaInfo.kmap_rad = atan(sqrt(tan(az).^2 + (tan(alt).^2) ./ (cos(az).^2))) * 180 / pi;
